...
export const test = baseTest.extend<CombinedFixtures>({
  ...
  djToken: [
    async ({ request }, use, testInfo) => {
      const stepCounter = new StepCounter();
      const tcId = resolveTcId(testInfo);
      const apiRequest = new ApiContextWrapper(request.raw(), testInfo, stepCounter);

      await log('INFO', fmt(tcId, 'Fetching djToken from dummyjson.com/auth/login'));

      const response = await apiRequest.post('https://dummyjson.com/auth/login', {
        data: {
          username: 'emilys',
          password: 'emilyspass'
        }
      ...
      const responseData = await response.json<{ accessToken?: string }>();
      const accessToken = responseData.accessToken;

      if (!accessToken) {
        const body = await response.safeBody();
        throw new Error(`Failed to get djToken. Response: ${body}`);
      }

      await log('INFO', fmt(tcId, `djToken acquired: ${accessToken.substring(0, 20)}...`));
  ...
  fakeToken: [
    async ({ request }, use, testInfo) => {
      ...
      const response = await apiRequest.post('https://fakestoreapi.com/auth/login', {
        data: {
          username: 'johnd',
          password: 'm38rmF$'
        }
      ...
      const responseData = await response.json<{ token?: string }>();
      const fakeToken = responseData.token;
      ...
      await log('INFO', fmt(tcId, `djToken acquired: ${fakeToken.substring(0, 20)}...`));
  ...
  decodedPayload: [
    async ({ djToken }, use, testInfo) => {
      const tcId = resolveTcId(testInfo);

      await log('INFO', fmt(tcId, 'Decoding JWT payload from djToken'));

      try {
        // JWT format: header.payload.signature
        const parts = djToken.split('.');
        if (parts.length !== 3) {
        ...
        await log('INFO', fmt(tcId, `JWT payload decoded successfully: ${Object.keys(payload).join(', ')}`));

        await use(payload);
      } catch (error) {
        const errMsg = error instanceof Error ? error.message : String(error);
        await log('ERROR', fmt(tcId, `Failed to decode JWT payload: ${errMsg}`));
        throw error;
      }
    },
    { scope: 'test' },
